...
>
> ⚠️ **事後稽核註記（2026-08-24 加，原句未改）：上面這句有兩個問題。**
> **①算術對不上**：9 + 1 + 14 = **24**，不是 21。記憶正本 `001_memory/project_stocksignal.md:196` 給的是另一種切法
> ——「效能 5 + 正確性 13 + 可簡化 3 = 21 項」，那是**議題類型**軸；A/B/C 是**處置**軸。兩軸的總數不該直接互相套用。
> C 類 14 項可由本檔自己的編號 1～14 佐證；A 類 9 / B 類 1 無獨立佐證。
> **②「git log 可查 `bc1b63a` 當時版本」是假的**：`git ls-tree --name-only bc1b63a | grep -i solomon` **輸出為空**
> ——該 commit 的樹裡根本沒有 `SOLOMON_HANDOFF.md`。加上該檔採「每輪覆寫不累加」，
> **原始 21 項清單的正本已滅失，這條命中無法完全定案。**
